...
import { Plus, TrendingUp, TrendingDown, Trash, DollarSign, CheckCircle } from "lucide-react";
...
export default function Financas() {
  ...
  const handleAddTransaction = (e: React.FormEvent) => {
    ...
    setAmount("");
    setIsAdding(false);
    showToast(`${type === "Receita" ? "Receita" : "Despesa"} lançada com sucesso!`, "success");
  };
  ...
  const handleConnectOpenFinance = () => {
    ...
    setTimeout(() => {
      ...
      setIsSyncing(false);
      setSyncSuccess(true);
      showToast(`Conta do ${selectedBank} conectada via Open Finance!`, "success");
    }, 2000);
  };
  ...
  return (
    <main className="p-6 md:p-10 min-h-screen">
      <PageHeader 
        title="Finanças" 
        description="Monitore seu saldo, receitas e despesas mensais"
      >
        <button 
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="glass-card rounded-2xl p-5 border border-white/5 flex items-center gap-4 bg-[#191b23]/30">
      ...
      <div className="glass-card rounded-2xl p-6 border border-white/5 bg-[#191b23]/30">
        <h2 className="text-lg font-bold mb-4">Histórico de Transações</h2>
        
        {expenses.length === 0 ? (
          <EmptyState 
            message="Nenhuma transação lançada" 
            description="Crie lançamentos manuais ou conecte seus cartões usando o Open Finance."
                ...
                {expenses.map((t) => (
                  <tr key={t.id} className="hover:bg-[#32353c]/5">
                    ...
                    <td className="py-4 text-right">
                      <button onClick={() => handleDeleteExpense(t.id)} className="text-on-surface-variant hover:text-error transition-colors">
      ...
      <Modal isOpen={isAdding} onClose={() => setIsAdding(false)} title="Novo Lançamento">
      ...
      <Modal isOpen={isOpenFinanceModal} onClose={handleCloseOpenFinance} title="Conexão Open Finance">
        {syncSuccess ? (
